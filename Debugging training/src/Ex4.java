...
public class Ex4 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < diasSemana.length; i++) {
            if (diasSemana[i].equals(diaEscolhido)) { //BUG (Comparação de string deve ser feita com "equal()", e não "==". No fim a execução levará ao mesmo resultado, mas queremos que sejá executado com eficiência e não com erros ou bug futuros.)
                indiceDia = i;
                break;
            }
        }
        ...
        soma = 0;
        for (int i = 0; i < 7; i++) {
            soma += temperaturas[i][hora]; //BUG (A posição "[horas][i]" estavam invertidas, temperatura é uma matriz "temperatura na hora do dia.)
        }
